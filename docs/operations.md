...
- **Gaia:** from summary `[GAIA METADATA]`. Auto-requery TAP when Teff/logg/MH are NaN, parallax is invalid, summary is missing, or `--force-redownload`. Atmosphere labels coalesce `gaia_source` GSP-Phot → `astrophysical_parameters` GSP-Phot → MSC primary (`teff_msc1` / `logg_msc1` / `mh_msc`) when earlier sources are null. Finite TAP fields are written back to the summary; still-NaN fields are **not** written (solar defaults apply in memory only: Teff=5500, log g=4, [Fe/H]=0, Mass=1).
